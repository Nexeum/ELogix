Overview
--------
ELogix (as in the beer) is a logging framework for Erlang. Its purpose is
...
Usage
-----
To use ELogix in your application, you need to define it as a rebar dep or have
...
Before logging any messages, you'll need to start the ELogix application. The
ELogix module's `start` function takes care of loading and starting any dependencies
ELogix requires.
...
You can also start ELogix on startup with a switch to `erl`:

```erlang
erl -pa path/to/lager/ebin -s lager
```

Once you have built your code with ELogix and started the ELogix application,
...
Configuration
-------------
To configure ELogix's backends, you use an application variable (probably in
your app.config):
...
Sinks
-----
ELogix has traditionally supported a single sink (implemented as a
`gen_event` manager) named `lager_event` to which all backends were
connected.

ELogix now supports extra sinks; each sink can have different
sync/async message thresholds and different backends.

### Sink configuration

To use multiple sinks (beyond the built-in sink of ELogix and ELogix_event), you
need to:
...
In `app.config` for the project that requires ELogix, for example,
extend the ELogix configuration to include an `extra_sinks` tuple with
...
* Atoms in the configuration are treated as placeholders for ELogix metadata and
...
------------------------
ELogix is also supplied with a `error_logger` handler module that translates
traditional erlang error messages into a friendlier format and sends them into
ELogix itself to be treated like a regular ELogix log call. To disable this, set
the ELogix application variable `error_logger_redirect` to `false`.
...
Prior to ELogix 2.0, the `gen_event` at the core of ELogix operated purely in
synchronous mode. Asynchronous mode is faster, but has no protection against
message queue overload. As of ELogix 2.0, the `gen_event` takes a hybrid
...
problems. In general, ELogix should process messages as fast as they come in, so getting
...
"Unsafe"
--------
The unsafe code pathway bypasses the normal ELogix formatting code and uses the
...
use the normal ELogix message formatting code which will provide the appropriate
...
------------------------
You can change the log level of any ELogix backend at runtime by doing the
following:
...
ELogix keeps track of the minimum log level being used by any backend and
...
---------------------
ELogix can rotate its own logs or have it done via an external process. To
...
This tells ELogix to log error and above messages to `error.log` and to
...
Syslog Support
--------------
ELogix syslog output is provided as a separate application:
...
Older Backends
--------------
ELogix 2.0 changed the backend API, there are various 3rd party backends for
ELogix available, but they may not have been updated to the new API. As they
...
----------------------
ELogix's parse transform will keep track of any record definitions it encounters
and store them in the module's attributes. You can then, at runtime, print any
record a module compiled with the ELogix parse transform knows about by using the
...
-----------------------
If you have Erlang R16 or higher, you can tell ELogix's console backend to be colored. Simply
add to ELogix's application environment config:
...
Tracing
-------
ELogix supports basic support for redirecting log messages based on log message
attributes. ELogix automatically captures the pid, module, function and line at the
...
As of ELogix 2.0, you can also use a 3 tuple while tracing, where the second
...
If no sink is thus specified, the default ELogix sink will be used.
...
can be fixed by rearchitecting ELogix's file backend, but this has not
been tackled.

### Traces from configuration

ELogix supports starting traces from its configuration file. The keyword
...
**IMPORTANT**: You **must** define a severity level in all ELogix releases
up to and including 3.1.0 or previous. The 2-tuple form wasn't added until
3.2.0.

Setting the truncation limit at compile-time
--------------------------------------------
ELogix defaults to truncating messages at 4096 bytes, you can alter this by
...
3.1.0 - 27 January 2016
    ...
    * Feature: API calls to a rotate handler, sink or all.  This change
      introduces a new `rotate` message for 3rd party ELogix backends; that's
